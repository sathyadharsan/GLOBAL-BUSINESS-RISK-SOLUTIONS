    ...
    <Card className="border border-slate-200 hover:shadow-lg transition-shadow">
      <CardContent className="p-6">
        <div className="flex items-start justify-between mb-3">
          <Badge variant="outline" className="text-sm" style={{ color: risk.color, borderColor: risk.color }}>
            #{risk.rank}
          </Badge>
          <TrendIndicator trend={risk.trend} />
        </div>
        <h3 className="font-bold text-primary mb-1">{risk.label}</h3>
        <p className="text-sm text-slate-500 mb-3">{risk.category}</p>
        ...
        <p className="text-sm text-slate-400 mt-2">Risk Score: {risk.score}/100</p>
      </CardContent>
    </Card>
  );
}
...
function TrendIndicator({ trend }: { trend: "up" | "down" | "stable" }) {
  if (trend === "up") return <span className="text-red-500 text-sm font-medium">↑ Rising</span>;
  if (trend === "down") return <span className="text-green-500 text-sm font-medium">↓ Declining</span>;
  return <span className="text-slate-500 text-sm font-medium">→ Stable</span>;
}
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6">
      <div className="grid grid-cols-6 gap-2">
        ...
        {likelihoodLabels.map((label, i) => (
          i > 0 && <div key={i} className="text-sm font-semibold text-center text-slate-600">{label}</div>
        ))}
        {Array.from({ length: 5 }).map((_, rowIdx) => (
          <React.Fragment key={rowIdx}>
            <div className="text-sm font-semibold text-slate-600 flex items-center justify-end pr-2">
            ...
            {Array.from({ length: 5 }).map((_, colIdx) => {
              ...
              return (
                <div
                  key={colIdx}
                  className="aspect-square rounded border border-slate-100 flex items-center justify-center text-sm font-medium text-white"
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
        ...
        <p className="text-sm text-slate-500">Likelihood → | Impact ↓</p>
      </div>
    </div>
  );
}